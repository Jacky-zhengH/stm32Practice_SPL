...
void Serial_Init(void)
{
	...
	GPIO_Instructer.GPIO_Mode=GPIO_Mode_AF_PP;      //复用功能输出，低电平触发，推挽输出
	GPIO_Instructer.GPIO_Pin=GPIO_Pin_9;
	GPIO_Instructer.GPIO_Speed=GPIO_Speed_50MHz;
	GPIO_Init(GPIOA,&GPIO_Instructer);
    /*接受口*/
	GPIO_Instructer.GPIO_Mode=GPIO_Mode_IPU;        //输入模式，可选择上拉输入或浮空输入
	GPIO_Instructer.GPIO_Pin=GPIO_Pin_10;
	GPIO_Instructer.GPIO_Speed=GPIO_Speed_50MHz;
	GPIO_Init(GPIOA,&GPIO_Instructer);
    ...
    USART_InitStructure.USART_Mode=USART_Mode_Tx |USART_Mode_Rx;                    //选择发送或接受模式，可用|同时触发
    ...
    //串口接受有两种方式：查询与中断
    //查询模式只需不断检测接受标志位的状态来判断是否有在接受数据
    //中断模式则需要再配置一个中断函数，接受一接受到数据就发出中断信号，触发中断信息

    /*USART中断配置*/
    USART_ITConfig(USART1,USART_IT_RXNE,ENABLE);       //开启USART标志位中断
    //NVIC配置部分
...
//在这里封装一个自己的标志位获取和数据读取函数有利于多字节的数据获取
